...
func Init(r *fiber.App) {
	...
	admin := v1.Group("/admin", middleware.Auth, middleware.IsAdmin)
	cashier := v1.Group("/cashier", middleware.Auth)
	owner := v1.Group("/owner", middleware.Auth, middleware.IsOwner)
	...
	//! * Kategori Routes 				*
	//! ***************************
	v1.Get("/categories", middleware.Auth, handler.GetKategori)

	cashier.Get("/kategori", handler.GetKategori)
